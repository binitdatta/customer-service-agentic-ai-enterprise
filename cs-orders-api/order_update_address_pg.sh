...
SHIP_TO_ADDRESS_ID="${SHIP_TO_ADDRESS_ID:-777}"
API_URL="${API_URL:-http://localhost:6061/api/orders/${ORDER_NUMBER}/shipping-address}"
...
cache_file_path() { echo "${CACHE_DIR}/$(safe_key "kc_${KC_REALM}_${CLIENT_ID}_${USERNAME:-svc}_${REQUESTED_SCOPE}").json"; }
json_get_access_token(){ local j="$1"; if have_cmd jq; then jq -r '.access_token // empty'<<<"$j"; else echo "$j"|sed -n 's/.*"access_token"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p'; fi; }
...
fetch_token() {
  ...
  if [[ "$GRANT_TYPE" == "password" ]]; then
    ...
    resp="$(curl -sS -X POST "$KC_TOKEN_URL" -H "Content-Type: application/x-www-form-urlencoded" \
    ...
    [[ -n "${CLIENT_SECRET:-}" ]] || { echo "ERROR: set CLIENT_SECRET" >&2; exit 1; }
    resp="$(curl -sS -X POST "$KC_TOKEN_URL" -H "Content-Type: application/x-www-form-urlencoded" \
...
token_from_cache_if_fresh() {
  ...
  local cf; cf="$(cache_file_path)"; [[ -f "$cf" ]] || return 1
  local now mtime age; now="$(date +%s)"
  if stat -f %m "$cf" >/dev/null 2>&1; then mtime="$(stat -f %m "$cf")"; else mtime="$(stat -c %Y "$cf")"; fi
  age=$(( now - mtime )); (( age <= CACHE_MAX_AGE_SECONDS )) || return 1
  cat "$cf"
}
...
echo "New ship_to_addr : $SHIP_TO_ADDRESS_ID"
...
echo "Calling update shipping address..."
HTTP_CODE="$(curl -sS -o /tmp/order_update_address_resp.json -w "%{http_code}" -X PATCH "$API_URL" \
...
echo "API response:"
cat /tmp/order_update_address_resp.json
echo
